...
class tail
{
    static void Main(string[] args)
    {
        ...
        if (hasOptions)
        {
            ...
            {
                case "-n":
                    if (fileNames.Length > 1 && int.TryParse(fileNames[1], out int n))
                        lineCount = n;
                    // 옵션 처리 후 파일명 재조정
                ...
                case "-c":
                    if (fileNames.Length > 1 && int.TryParse(fileNames[1], out int c))
                        byteCount = c;
                    // 옵션 처리 후 파일명 재조정
                    fileNames = fileNames[2..];
                    break;
